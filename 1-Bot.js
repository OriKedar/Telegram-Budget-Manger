class TelegramBot {
  ...
  addReplyKeyboardMarkup(elements = []) {
  ...
  addReplyKeyboardRemove() {
    return JSON.stringify({remove_keyboard: true});
  }

  isUserAuthorized(id){
    return this.authorizedUsersList.includes(id.toString());
  }

  updateActiveUsersList(){
    return this.activeUsers = getActiveUsersIds()
  }

  isActiveUser(id){
    return this.activeUsers.includes(id);
  }
  ...
  /**
   * Override the "sendMessage" method in order to manage authorized users
   */
  sendMessageToActiveUser(id, text, keyboard = null) {
    if(this.isActiveUser(id)){
      this.sendMessage(id, text, keyboard)
    } else {
      return this.sendMessage(id, "You are not active user\n Try to use /cancel and then /start to register to the service");
    }
  }
